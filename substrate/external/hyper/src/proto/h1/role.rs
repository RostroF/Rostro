        ...
        for header in &headers_indices[..headers_len] {
            ...
            match name {
                header::TRANSFER_ENCODING => {
                    ...
                    // F-LAB-RT-05 closure (2026-05-26, Rostro fork): if a
                    // Content-Length was already accepted on this request,
                    // we now have BOTH TE and CL — request-smuggling
                    // primitive per RFC 7230 §3.3.3 rule 3. Upstream hyper
                    // 1.6.0 silently prefers TE and drops CL via the
                    // `if is_te { continue }` short-circuit on the CL arm
                    // below, leaving the smuggling vector live whenever an
                    // upstream proxy uses CL. Reject hard with 400.
                    if con_len.is_some() {
                        debug!(
                            "RFC 7230 §3.3.3: request with both Transfer-Encoding \
                             and Content-Length (Rostro: F-LAB-RT-05)",
                        );
                ...
                header::CONTENT_LENGTH => {
                    // F-LAB-RT-05 closure (2026-05-26, Rostro fork): the
                    // upstream `if is_te { continue }` short-circuit hid the
                    // both-headers smuggling primitive. Reject hard instead
                    // — same shape as the differing-CL-values check below.
                    if is_te {
                        debug!(
                            "RFC 7230 §3.3.3: request with both Transfer-Encoding \
                             and Content-Length (Rostro: F-LAB-RT-05)",
...
mod tests {
    ...
    fn test_decoder_request() {
        ...
        // F-LAB-RT-05 closure (2026-05-26, Rostro fork): the three test
        // cases below previously codified upstream hyper 1.6.0's vulnerable
        // behavior — they asserted that a request with BOTH
        // Transfer-Encoding and Content-Length was silently accepted with
        // TE winning. That IS the request-smuggling primitive (RFC 7230
        // §3.3.3 rule 3 says such requests "ought to be handled as an
        // error"). The Rostro fork now rejects them with
        // ContentLengthInvalid → 400 Bad Request; these tests flipped from
        // `assert_eq!(parse(…).decode, CHUNKED)` to `parse_err(…)`.
        parse_err(
            "\
             POST / HTTP/1.1\r\n\
             content-length: 10\r\n\
             ...
             \r\n\
             ",
            "TE+CL both present (CL first) — request-smuggling primitive",
        );
        ...
        parse_err(
            "\
             ...
             \r\n\
             ",
            "TE+CL both present (TE first) — request-smuggling primitive",
        );
        ...
        parse_err(
            "\
             ...
             \r\n\
             ",
            "TE+CL both present (TE-CL-TE interleaved) — request-smuggling primitive",
        );
