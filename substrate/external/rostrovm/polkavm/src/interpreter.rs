...
pub const MAX_BLS_MSM: u32 = 2048;
...
/// (uncompressed affine). Points validated on deserialize; scalars are
/// canonical 32-byte Fr encodings. Returns `false` on any malformed input.
pub fn rostro_bls381_g1_msm(points: &[u8], scalars: &[u8], n: usize, out: &mut [u8; 96]) -> bool {
    ...
    for i in 0..n {
        let Ok(p) = G1Affine::deserialize_uncompressed(&points[i * 96..(i + 1) * 96]) else {
            return false;
        };
    ...
    let Ok(acc) = G1Projective::msm(&ps, &ss) else { return false };
...
pub fn rostro_bls381_g2_msm(points: &[u8], scalars: &[u8], n: usize, out: &mut [u8; 192]) -> bool {
    ...
    let mut ss = Vec::with_capacity(n);
    for i in 0..n {
        let Ok(p) = G2Affine::deserialize_uncompressed(&points[i * 192..(i + 1) * 192]) else {
            return false;
        };
        let Ok(s) = Fr::deserialize_uncompressed(&scalars[i * 32..(i + 1) * 32]) else {
            return false;
    ...
    let Ok(acc) = G2Projective::msm(&ps, &ss) else { return false };
    acc.into_affine().serialize_uncompressed(&mut out[..]).is_ok()
}
