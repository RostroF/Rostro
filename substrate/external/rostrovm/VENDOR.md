...
| Montgomery-limb MSM intrinsics (`G1_MSM_MONT = 131`, `G2_MSM_MONT = 132`, `BANDERSNATCH_TE_MSM_MONT = 133`, `SW_MSM_MONT = 134`) | `polkavm/src/interpreter.rs`, `lib.rs`, `tests/kat_vectors.rs` | Same MSMs as 115/116/119/125 but points/scalars travel as raw LE Montgomery limbs via public ark API (`Fp.0`/`new_unchecked` — no repr bets), eliminating the constant ~30 µs/point interpreted Montgomery↔bytes conversion that dominates the byte ABI as n grows. Limbs ≥ modulus REJECTED (fail closed: `new_unchecked` on non-canonical residues overflows in debug builds — a debug/release divergence otherwise). SW infinities filtered by caller; all-zero out = identity; TE identity (0,1) representable. The hooks' MSM path; byte arms remain wire-facing. | Tier 2 |
