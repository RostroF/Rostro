...
impl<T: AsyncRead + AsyncWrite + Unpin> Receiver<T> {
	...
	pub async fn receive(&mut self, message: &mut Vec<u8>) -> Result<Incoming<'_>, Error> {
		...
		loop {
			...
			// F-LAB-RT-04 closure (2026-05-26, Rostro fork): RFC 6455 §5.1
			// requires that clients mask every frame sent to the server, and
			// that servers MUST close the connection upon receiving an
			// unmasked frame. Upstream soketto 0.8.0 silently accepts
			// unmasked client frames (the mask bit is parsed at base.rs:416
			// but never enforced on the receive path; `apply_mask` is a
			// no-op when the bit is unset). The sender side already enforces
			// the symmetric invariant (line ~553 sets the mask bit for client
			// frames), so this restores symmetry.
			if self.mode.is_server() && !header.is_masked() {
				log::debug!(
					"{}: RFC 6455 §5.1 violation: unmasked frame from client \
					 (opcode={:?}); closing connection",
					self.id,
					header.opcode(),
				);
				self.is_closed = true;
...
pub enum Error {
	...
	Closed,
	/// RFC 6455 §5.1 violation: a server-mode `Receiver` got an unmasked
	/// frame from the client. Rostro-fork addition (F-LAB-RT-04 closure);
	/// upstream soketto 0.8.0 silently accepts unmasked client frames.
	UnmaskedClientFrame,
}
	...
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match self {
			...
			Error::UnmaskedClientFrame => {
				f.write_str("RFC 6455 §5.1: unmasked frame from client (Rostro: F-LAB-RT-04)")
			}
		}
	}
}
...
impl std::error::Error for Error {
	fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
		match self {
			...
			| Error::Closed
			| Error::UnmaskedClientFrame => None,
		}
	}
}
...
#[cfg(test)]
mod tests {
	use super::{discard_bytes, Error, Mode};
	...
	/// F-LAB-RT-04 regression (Rostro fork, 2026-05-26): RFC 6455 §5.1
	/// requires a server to close the connection upon receiving an
	/// unmasked frame from the client. Upstream soketto 0.8.0 silently
	/// accepts unmasked frames; this test pins the new server-side
	/// enforcement so a future refactor doesn't quietly undo it.
	#[tokio::test]
	async fn server_rejects_unmasked_client_frame() {
		// Hand-crafted WebSocket frame bytes for a 5-byte unmasked Text
		// frame ("hello"). Header byte 1: 0x81 = FIN + opcode 0x1 (Text).
		// Header byte 2: 0x05 = mask bit clear, payload length 5.
		// Followed by the 5 payload bytes "hello".
		let unmasked_frame: Vec<u8> = vec![0x81, 0x05, b'h', b'e', b'l', b'l', b'o'];

		let cursor = Cursor::new(unmasked_frame);
		let (_sender, mut receiver) = super::Builder::new(cursor, Mode::Server).finish();
		let mut message = Vec::new();

		match receiver.receive(&mut message).await {
			Err(Error::UnmaskedClientFrame) => {}
			Err(other) => panic!("expected Error::UnmaskedClientFrame, got {other:?}"),
			Ok(incoming) => panic!(
				"expected RFC 6455 §5.1 rejection of unmasked client frame; \
				 got Ok({incoming:?}) — F-LAB-RT-04 has regressed"
			),
		}
	}

	/// Companion: confirm the symmetric MASKED case still works for a
	/// server-mode receiver, so the unmasked-frame check isn't accidentally
	/// rejecting all client frames.
	#[tokio::test]
	async fn server_accepts_masked_client_frame() {
		// Same "hello" payload, but masked. Header byte 1: 0x81 (FIN +
		// Text). Header byte 2: 0x85 (mask bit set, payload len 5).
		// 4-byte mask key (arbitrary): 0xde 0xad 0xbe 0xef. Payload XORed
		// with the mask byte-by-byte:
		//   'h'(0x68) ^ 0xde = 0xb6
		//   'e'(0x65) ^ 0xad = 0xc8
		//   'l'(0x6c) ^ 0xbe = 0xd2
		//   'l'(0x6c) ^ 0xef = 0x83
		...
		let masked_frame: Vec<u8> = vec![
			0x81, 0x85, 0xde, 0xad, 0xbe, 0xef, 0xb6, 0xc8, 0xd2, 0x83, 0xb1,
		];

		let cursor = Cursor::new(masked_frame);
		let (_sender, mut receiver) = super::Builder::new(cursor, Mode::Server).finish();
		let mut message = Vec::new();

		match receiver.receive(&mut message).await {
			Ok(_) => assert_eq!(&message, b"hello"),
			Err(e) => panic!(
				"server should accept masked client frames per RFC 6455 §5.1; \
				 got {e:?} — masked-case regression",
			),
		}
	}
}
