...
macro_rules! load_json_file {
    ($json_file:expr) => {{
        // Rostro vendor change (see VENDOR.md): vectors vendored locally
        // instead of upstream's repo-root git submodule.
        let path = format!("tests/wycheproof-vectors/{}", $json_file);
...
macro_rules! mlkem_test {
    ($name:ident, $json_file:expr, $kem:ident, $kem_module:ident) => {
        #[test]
        fn $name() {
            ...
            for group in tests.groups {
                ...
                for test in &group.tests {
                    ...
                    let test_seed = match decode_optional_hex(&test.seed, "seed") {
                        Some(seed) => seed,
                        None => {
                    ...
                    let decrypted_k = dk.decapsulate(&test_c);
                    assert_eq!(test_k, decrypted_k);
                }
            }
        }
    };
...
// Rostro vendor change (see VENDOR.md): only ML-KEM-768 vectors are
// vendored — Rostro ships exactly one parameter set. The 512/1024 test
// invocations are removed with their data; the macros above are
// untouched, so restoring them is vector-download plus reverting this
// block.
mlkem_test!(mlkem_768_test, "mlkem_768_test.json", MlKem768, ml_kem_768);
