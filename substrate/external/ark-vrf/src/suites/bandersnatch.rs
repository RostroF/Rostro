...
impl Suite for ThisSuite {
    ...
    // ROSTRO DEVIATION (see VENDOR.md): the suite runs on the HOOKED
    // bandersnatch — group operations route through RostroCurveHooks
    // (ecalli intrinsics in-guest, plain-arkworks transmute natively).
    // Field types, curve constants and serialization are identical to the
    // plain curve; the byte-equality suite pins it.
    type Affine = rostro_curve_hooks::EdwardsAffine;
    ...
    fn data_to_point(data: &[u8]) -> Option<AffinePoint> {
        ...
        // ROSTRO DEVIATION: the map runs on the PLAIN config and the result
        // is coordinate-copied into the hooked type. The ext config cannot
        // implement ark-ec's `Elligator2Config` (orphan rule), and the map
        // is pure field arithmetic — no group operations — so there is
        // nothing for the hooks to accelerate. Same field types, same DST,
        // same constants: byte-identical output by construction.
        use ark_ec::hashing::{
            HashToCurve, curve_maps::elligator2::Elligator2Map,
            map_to_curve_hasher::MapToCurveBasedHasher,
        };
        use ark_ff::field_hashers::DefaultFieldHasher;
        let dst: ark_std::vec::Vec<_> =
            [b"ECVRF_" as &[u8], h2c_suite_id, Self::SUITE_ID].concat();
        let p = MapToCurveBasedHasher::<
            ark_ed_on_bls12_381_bandersnatch::EdwardsProjective,
            DefaultFieldHasher<<Self as Suite>::Hasher, 128>,
            Elligator2Map<ark_ed_on_bls12_381_bandersnatch::BandersnatchConfig>,
        >::new(&dst)
        .and_then(|hasher| hasher.hash(data))
        .ok()?;
        Some(AffinePoint::new_unchecked(p.x, p.y))
    }
}
...
impl crate::ring::RingSuite for ThisSuite {
    // ROSTRO DEVIATION: the ring-proof KZG backend runs on the HOOKED
    // BLS12-381 engine — MSM, Miller loop and final exponentiation route
    // through RostroCurveHooks.
    type Pairing = rostro_curve_hooks::Bls12_381;
