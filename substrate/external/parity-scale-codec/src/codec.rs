...
{
	// Rostro vendor change (see VENDOR.md): upstream const-asserted
	// `INITIAL_PREALLOCATION >= size_of::<T>()`, forbidding Vec elements
	// larger than 16 KiB at compile time. Rostro's hybrid finality-vote
	// types (ed25519 + SLH-DSA-SHA2-128f) are ~17.2 KiB, so oversized
	// elements decode in chunks of one item instead of being unbuildable.
	// The assert existed to prevent a zero chunk length (infinite loop);
	// the `.max(1)` clamp preserves that guarantee.
	//
	// we have to account for the fact that `mem::size_of::<T>` can be 0 for types like `()`
	// for example.
	let mut chunk_len = match mem::size_of::<T>() {
		0 => 1,
		size => (INITIAL_PREALLOCATION / size).max(1),
	};
