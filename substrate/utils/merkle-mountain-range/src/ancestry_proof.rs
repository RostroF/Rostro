...
use crate::helper::{
    get_peak_map, get_peaks, is_canonical_mmr_size, is_descendant_pos, leaf_index_to_pos,
...
impl<T: Clone + PartialEq, M: Merge<Item = T>> NodeMerkleProof<T, M> {
    ...
    pub fn calculate_root(&self, leaves: Vec<(u64, T)>) -> Result<T> {
        // Rostro hardening (C): reject non-canonical mmr_size (see helper::is_canonical_mmr_size).
        if !is_canonical_mmr_size(self.mmr_size) {
            return Err(Error::CorruptedProof);
        }
        // NB: empty `leaves` are intentionally permitted here — `NodeMerkleProof::verify_ancestor`
        // calls into `verify` with `leaves` derived from prev-peaks (via the prev_peaks_proof
        // self-reference), and that path has its own structural sanity checks. The
        // empty-leaves rejection lives at `MerkleProof::verify` / `MerkleProof::calculate_root`,
        // which is the surface external callers actually use to verify a leaf-inclusion proof.
...
>(
    ...
    // Rostro hardening (A) mirror: track that we account for every input node across
    // all peaks. The original count check below is the only safety net catching
    // Hyperbridge-style attacker leaves whose pos is past the last peak; we add a
    // count-equality check as a redundant second layer.
    let original_nodes_count = nodes.len();
    let mut consumed_nodes: usize = 0;
    let mut bagged_break = false;

    let mut peaks_hashes: Vec<T> = Vec::with_capacity(peaks.len() + 1);
    for peak_pos in peaks {
        let mut nodes_for_peak: Vec<(u64, T)> =
            take_while_vec(&mut nodes, |(pos, _)| *pos <= peak_pos);
        let nodes_for_peak_len = nodes_for_peak.len();
        let peak_root = if nodes_for_peak.len() == 1 && nodes_for_peak[0].0 == peak_pos {
            // leaf is the peak
            nodes_for_peak.remove(0).1
        } else if nodes_for_peak.is_empty() {
            // Rostro hardening (E): the upstream `break`s here on the assumption that
            // subsequent peaks are accounted for by absent proof items. That fallthrough-
            // accept pattern is the structural shape of the Hyperbridge Solidity bug;
            // the only safety net is the `nodes.len() != 0` check below. Mark that the
            // loop was cut short and validate post-loop that ALL nodes were drained,
            // providing a second layer of defense against silent truncation.
            bagged_break = true;
            break;
        } else {
            calculate_peak_root::<_, M>(nodes_for_peak, peak_pos)?
        };
        consumed_nodes += nodes_for_peak_len;
        peaks_hashes.push(peak_root.clone());
    }

    // Rostro hardening (A) / (E): if we broke out early (no further proof items), we
    // must not have any remaining nodes — otherwise an attacker could smuggle leaves
    // past unconsumed peaks.
    if bagged_break && !nodes.is_empty() {
        return Err(Error::CorruptedProof);
    }
    ...
    // attack now requires fooling BOTH the emptiness check above AND this count check.
    if consumed_nodes != original_nodes_count {
        return Err(Error::CorruptedProof);
    }
