...
//!
//! An equivalent of `rp_io::TestExternalities` that can load its state from a remote substrate
...
use rp_state_machine::TestExternalities;
use std::{
...
/// An externalities that acts exactly the same as [`rp_io::TestExternalities`] but has a few extra
	...
	B::Header: DeserializeOwned,
{
	...
	async fn init_remote_client(&mut self) -> Result<()> {
		...
		online_config.pallets.iter().for_each(|p| {
			online_config
				.hashed_prefixes
				.push(rp_crypto_hashing::twox_128(p.as_bytes()).to_vec())
		});
	...
	pub(crate) async fn pre_build(mut self) -> Result<RemoteExternalities<B>> {
		...
		if !self.hashed_blacklist.is_empty() {
			...
			for k in self.hashed_blacklist {
				ext.execute_with(|| rp_io::storage::clear(&k));
			}
		}
...
mod test_prelude {
	pub(crate) use super::*;
	pub(crate) use rp_runtime::testing::{Block as RawBlock, MockCallU64};
	pub(crate) type UncheckedXt = rp_runtime::testing::TestXt<MockCallU64, ()>;
...
mod tests {
	...
	async fn can_exclude_from_snapshot() {
		...
		let some_key = Builder::<Block>::new()
			...
			.execute_with(|| {
				let key =
					rp_io::storage::next_key(&[]).expect("some key must exist in the snapshot");
				assert!(rp_io::storage::get(&key).is_some());
				key
			});
		...
		Builder::<Block>::new()
			...
			.await
			.expect("Can't read state snapshot file")
			.execute_with(|| assert!(rp_io::storage::get(&some_key).is_none()));
	}
}
...
mod remote_tests {
	...
	async fn child_keys_are_loaded() {
		...
		let child_info = rp_core::storage::ChildInfo::new_default(b"test_child");
