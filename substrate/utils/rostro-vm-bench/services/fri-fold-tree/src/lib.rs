...
//! indices in a flat 64 KB tree buffer — exposes cache + allocator pressure
//! that the composite verifier didn't isolate.
//!
//! Per-call work proportions:
//!   - Tree alloc + init: 8191 Goldilocks muls, sequential
//!   - Walk: 30 queries × 12 levels = 360 Poseidon2 perms + 720 Goldilocks
//!     muls + 360 scattered reads from the 64 KB tree buffer
...
mod bump_alloc {
	...
	const HEAP_SIZE: usize = 256 * 1024; // 256 KB — covers 64 KB tree + slack
...
const N: usize = 1 << TRACE_LOG; // 4096 leaves
const TREE_SIZE: usize = 2 * N - 1; // 8191 total nodes
...
pub fn fri_fold_tree_bench() -> u32 {
	// Build the synthetic tree buffer. Real FRI would have hashed
	// internals; we just fill with deterministic Goldilocks values
	// because the bench measures access patterns + per-step cost,
	// not commitment correctness.
	...
	// Per-level offsets into the flat tree buffer:
	//   level 0 (leaves): [0 .. N)
	//   level 1:          [N .. N + N/2)
	//   level l:          [offsets[l] .. offsets[l] + (N >> l))
	...
	// Derive transcript challenges deterministically.
	let mut state: [u64; 8] = [
		0xdeadbeef_00000000,
		0xdeadbeef_00000001,
		0xdeadbeef_00000002,
	...
	// Walk K queries from leaf to root, scattered sibling lookup at
	// each level.
	let mut accum = ZERO;
	let mut q = 0;
	while q < NUM_QUERIES {
		...
		while level < TRACE_LOG as usize {
			...
			// Mix current + sibling into a Poseidon2 sponge over the
			// transcript state, derive a challenge, fold with linear
			// combination.
			let mut h = state;
			h[0] = current;
			h[1] = sibling;
