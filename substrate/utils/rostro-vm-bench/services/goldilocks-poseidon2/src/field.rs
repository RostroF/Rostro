...
///
/// On polkavm: dispatched via the RostroVM `rostro_goldilocks_add` intrinsic
/// (ecalli index 101) — single inline dispatch + native body. On other
/// targets: pure-Rust overflow-check + EPSILON adjust.
#[inline(always)]
pub fn add(a: u64, b: u64) -> u64 {
	#[cfg(target_env = "polkavm")]
	{
		unsafe { crate::polkavm::rostro_goldilocks_add(a, b) }
	}
...
pub fn sub(a: u64, b: u64) -> u64 {
	#[cfg(target_env = "polkavm")]
	{
		unsafe { crate::polkavm::rostro_goldilocks_sub(a, b) }
	}
	#[cfg(not(target_env = "polkavm"))]
	{
...
pub fn mul(a: u64, b: u64) -> u64 {
	#[cfg(target_env = "polkavm")]
	{
		unsafe { crate::polkavm::rostro_goldilocks_mul(a, b) }
	}
	#[cfg(not(target_env = "polkavm"))]
	{
...
pub fn canonical(a: u64) -> u64 {
	if a >= P { a.wrapping_sub(P) } else { a }
}
...
/// over N elements.
///
/// On polkavm: dispatched via `rostro_goldilocks_inv` intrinsic (ecalli 103)
/// — one inline dispatch replaces the full ladder. ~30-75× speedup per inv.
#[inline]
pub fn inv(x: u64) -> u64 {
	#[cfg(target_env = "polkavm")]
	{
		unsafe { crate::polkavm::rostro_goldilocks_inv(x) }
	}
	#[cfg(not(target_env = "polkavm"))]
	{
		pow(x, P - 2)
