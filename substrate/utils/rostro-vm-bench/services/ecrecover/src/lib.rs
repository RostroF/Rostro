...
// Bump allocator for no_std targets. k256 needs alloc for internal operations;
// a single ecrecover uses bounded memory so we never need to dealloc.
#[cfg(target_os = "none")]
mod bump_alloc {
...
/// Recover the public key from a signature + message hash. Returns 1 if the
/// recovered key matches the expected public key, 0 otherwise.
