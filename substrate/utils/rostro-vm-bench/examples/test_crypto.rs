...
fn main() {
	...
	// dilithium_verify: 1 = signature verified, 0 = failed. The fixed counter
	// RNG + fixed message + fixed context produce a deterministic keygen+sign,
	// so all VMs (and the polkavm-interp intrinsic path) must return 1.
	check_workload(
		"dilithium_verify",
		DILITHIUM_JAVM_BLOB,
		DILITHIUM_POLKAVM_BLOB,
		DILITHIUM_WASM_BLOB,
		Some(1),
	);
	check_workload(
