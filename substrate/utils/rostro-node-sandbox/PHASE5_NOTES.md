...
| F-NEW-R4-FOLLOWUP-1 host mount-NS containment (architectural follow-up promoted to fix, comprehensive audit 2026-05-25, confidence 9/10) | DOCUMENTED RESIDUAL since Phase H (host-visible bind-remounts; user-verified live by accidentally noexec-remounting `/etc` on WSL) → **CLOSED via `install_mount_ns` (2026-05-25)** | `linux.rs` new `install_mount_ns` (target_arch=x86_64 only; non-x86 stub); called FIRST in `install()` between cgroup and noexec-remount; `unshare(CLONE_NEWNS)` + `mount(NULL, "/", NULL, MS_REC\|MS_PRIVATE, NULL)` | The architectural fix that the R2-04 + R3-03 denylists were stopgaps for. Empirically verified live: `--sandbox-rw-path /tmp/rostro-ns-test` bind-remount appears in supervisor's NS (via `nsenter -t <sup_pid> -m mount`) but `mount \| grep` on the host returns nothing. Subsequent supply-chain bind attempts (e.g., compromised `landlock`/`seccompiler` crate calling `mount(MS_BIND, "/")`) also stay contained. Operator UX caveat documented: use `nsenter` for in-NS mount inspection. `ROSTRO_SKIP_MOUNT_NS=1` diagnostic available. |
| F-NEW-R4-FOLLOWUP-3 cross-validator localhost RPC reach (architectural follow-up promoted to lab-side fix, comprehensive audit 2026-05-25, confidence 9/10) | DOCUMENTED RESIDUAL since R3-01 (F-NEW-08 in §3) → **CLOSED via per-host nftables stopgap in lab wrapper (2026-05-25)** | `playbooks/lab-supervisor-wrapper.sh` Step 3b installs per-role nftables table (`rostro-rpc-${ROLE}`) before exec'ing supervisor; rule: `meta skuid != $CHILD_UID iif "lo" tcp dport $RPC_PORT drop` (allow CHILD_UID's own loopback, drop others) | The cheap stopgap that the audit assessed as immediately tractable. Per-validator netns remains the architectural fix and is documented as deferred (significant libp2p discovery cost). Precondition: F-NEW-03 per-role-UID provisioning. Deployment prerequisite: `nft` installed (standard on Fedora 40+/Debian 12+/Ubuntu 24.04+). |
| F-NEW-R4-V2 pipe-relay unbounded line (NEW finding, audit 2026-05-25, LOW/8) | SUSPECTED REACHABLE (compromised child writes single multi-GiB line, supervisor RSS grows unboundedly) → **CLOSED via `bounded_read_until_newline` (2026-05-25)** | `main.rs` new helper caps per-line at `MAX_RELAY_LINE_BYTES` (64 KiB); on cap-hit appends `[cannae: line truncated]\n` marker; pipe-relay reader factored into `relay_reader_body` for testability + catch_unwind support | User-stated DOS exclusion notwithstanding, fix is 6 LOC and prevents the host-OOM-killer-induced supervisor restart that would degrade F-NEW-R3-01 counter-cap promise. 4 unit tests cover short-line, truncation-at-cap, EOF-without-newline, empty-input. |
| F-NEW-R4-V3 pipe-relay reader-thread panic isolation (NEW finding, audit 2026-05-25, LOW/8) | SUSPECTED REACHABLE (panic in reader thread → silent thread exit → child blocks on full pipe → block production stops without supervisor restart or forensics) → **CLOSED via catch_unwind + exit(2) (2026-05-25)** | `main.rs` reader thread body wrapped in `std::panic::catch_unwind`; on panic, panic-payload extracted to stderr and `std::process::exit(2)` terminates the whole supervisor for systemd ExecStart restart | Loud-fail-then-restart pattern beats silent-block. Aligns with systemd-watched supervisor expectations. |
| F-NEW-R4-V5 state-file ↔ stdio-log path collision (NEW finding, audit 2026-05-25, MEDIUM/9) | CONFIRMED REACHABLE (operator passes same path for `--state-file` + `--sandbox-stdio-log` → save_atomic rename orphans the open stdio fd → forensics broken silently) → **CLOSED via `validate_state_and_stdio_disjoint` (2026-05-25)** | `main.rs` new validator runs at parse time after R2-03 placement validation; normalizes both paths via `normalize_path_for_denylist` (R3-03) before equality check; refuses with F-NEW-R4-V5 error | Empirically verified live: `--state-file /var/log/foo --sandbox-stdio-log /var/log/foo` refused with F-NEW-R4-V5 error. Symmetric with F15/F16 overlap-rejection family. 5 unit tests cover None+None, distinct, exact collision, normalized collision (e.g., `/var/log/foo` vs `/var/log//foo`), R3-03 `..` propagation. |
| F-NEW-R4-V8 state.tmp open without O_NOFOLLOW (NEW finding, audit 2026-05-25, LOW/8) | CONFIRMED REACHABLE (operator misconfig + symlink-plant on state-file directory → supervisor [root] clobbers symlink target with serialized state content) → **CLOSED via explicit OpenOptions with O_NOFOLLOW (2026-05-25)** | `main.rs` `SupervisorState::save_atomic` switched from `std::fs::write` to explicit `OpenOptions::new().create(true).write(true).truncate(true).custom_flags(libc::O_NOFOLLOW).mode(0o600).open(&tmp)` | Symmetric with stdio mirror file's R2-02 O_NOFOLLOW + R4-01 mode 0600. Empirically verified live: planted `<state-file>.tmp → canary-target` symlink → supervisor save_atomic fails ELOOP ("Too many levels of symbolic links" os error 40); canary target NOT clobbered; symlink still in place; STATE_DELTA still emits to journald via R3-01 fallback. |
| F-NEW-R4-01b stdio-log denylist asymmetric with kernel-interface trees (NEW finding, follow-on /security-review 2026-05-25, confidence 8/10 after FP-filter) | CONFIRMED REACHABLE (operator passes `--sandbox-stdio-log /proc/sys/kernel/core_pattern` or `/proc/sys/kernel/modprobe` or `/sys/kernel/uevent_helper` or `/proc/sysrq-trigger` → supervisor opens AS ROOT with `O_APPEND\|O_NOFOLLOW, 0600` [none of which block procfs/sysfs] → pipe-relay reader appends child-controlled bytes verbatim → kernel itself executes the bytes on next crash/autoload/uevent → root sandbox escape; SHARPER than the R4-01 /etc/* family because no daemon-parser indirection) → **CLOSED via denylist extension (2026-05-25)** | `main.rs` `STDIO_LOG_SYSTEM_PREFIX_DENYLIST` extended with `/proc`, `/sys`, `/dev`, `/run` (now 13 entries, symmetric with `SYSTEM_TOPLEVEL_DENYLIST` kernel-interface coverage); `/dev/null` short-circuit at line ~380 still permits the legitimate discard target | The follow-on /security-review HIGH-severity finding (after the initial agent KEPT the original at confidence 8). The original R4-01 closure covered daemon-parsed config trees (`/etc/sudoers.d`, `/etc/cron.d`, …) but missed the kernel-interface trees that `SYSTEM_TOPLEVEL_DENYLIST` (R2-04) had already deemed system-managed. Asymmetric coverage is the smell. 8 new parameterized unit tests cover: `core_pattern`, `modprobe`, `sysrq-trigger`, `uevent_helper`, broader `/proc/sys/*` + `/sys/*` matrix, `/dev/{sda,mem,kmsg,random}` (with `/dev/null` sanity check that the legitimate discard target still works), `/run/{systemd/system/*,sudo/ts/*,cron.d/*}`, R3-03 normalization-bypass shapes against the new entries (`/proc//sys/...`, `/sys/./kernel/...`, `/var/log/../proc/sys/...`). Total: 25 `stdio_log_placement` tests, 108 supervisor tests pass. No live empirical re-verification needed (path validator is parse-time deterministic, deeply unit-tested); the test matrix is the verification. |
