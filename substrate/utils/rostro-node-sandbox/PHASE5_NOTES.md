...
| F-NEW-R2-04 `--sandbox-rw-path` top-level system dir (NEW finding, round-2 pen-test 2026-05-25) | CONFIRMED REACHABLE (operator misconfig `--sandbox-rw-path /etc` → host-global `MS_NOEXEC` remount breaks system services) → **PARTIALLY CLOSED via denylist (2026-05-25)** | `main.rs` new `validate_rw_paths_not_system_dirs` + `SYSTEM_TOPLEVEL_DENYLIST` const | Stopgap, not architectural. The denylist refuses exact-match of well-known top-level system directories (`/`, `/etc`, `/usr`, `/var`, `/tmp`, `/proc`, `/sys`, `/dev`, `/lib*`, `/boot`, `/root`, `/home`, `/srv`, `/opt`, `/mnt`, `/media`, `/run`); nested paths under those remain valid. The architectural fix is `unshare(CLONE_NEWNS)` to contain the noexec bind-remount to the supervisor's mount namespace — tracked for a separate decision because the UX tradeoff (host-visible mounts for operator debugging) deserves a deliberate call. 5 new unit tests cover the matrix. |
