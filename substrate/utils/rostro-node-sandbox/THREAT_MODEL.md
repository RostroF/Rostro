...
| F-NEW-R3-01 | Supervisor's own Landlock blocks state-file persist; `swap_count` + `crashes` degrade from per-deployment-lifetime caps to per-supervisor-lifetime, and an operator-triggered `systemctl restart` clears them | **F-NEW-R3 closure (2026-05-25):** structured `STATE_DELTA schema=1 swap_count=N crashes=[T1,T2,...]` lines emitted to supervisor stderr on every mutation. systemd's `ExecStart` forwards supervisor stderr → systemd-journald → `/var/log/journal/`, which sits outside Cannae's policy envelope and outside the child's DAC reach. journald is the canonical attacker-unreachable forensic mirror; state file remains the best-effort local cache. Persist-failure log tightened from WARN → ERROR with operator-facing reconstruction command (`journalctl -u rostro-supervisor --output=cat \| grep STATE_DELTA \| tail -1`). Architectural follow-up — Landlock-child-only refactor (move install_landlock to pre_exec) would restore disk persist but is a meaningful design change; deferred to a separate session. |
