...
| F-NEW-R4-FOLLOWUP-3 (cross-validator localhost RPC reach — lab-side stopgap) | Documented as `F-NEW-08` residual in earlier rounds: compromised validator-A on the same host as validator-B can `connect(127.0.0.1:9944)` to validator-B's RPC and call every "safe-by-default" method (pending-pool dump, extrinsic submission, chain queries). Phase G's CAP_KILL drop closes cross-UID signal but not cross-UID localhost TCP. | **F-NEW-R4 closure (2026-05-25)**, lab-deployment side: `playbooks/lab-supervisor-wrapper.sh` Step 3b installs a per-role nftables table (`rostro-rpc-<role>`) on each lab node before exec'ing the supervisor. Rule: `meta skuid != $CHILD_UID iif "lo" tcp dport $RPC_PORT drop` (matches the kernel-tracked source UID; cross-host TCP is unaffected since `skuid` only matches same-host connections). Allows the validator's own gemini-node (`meta skuid $CHILD_UID accept`). Per-role-UID provisioning (F-NEW-03) is the precondition. Per-host nftables is the cheap stopgap; per-validator netns remains the architectural fix and is documented as deferred (significant libp2p discovery cost). |
