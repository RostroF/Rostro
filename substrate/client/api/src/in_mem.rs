...
use rp_blockchain::{CachedHeaderMetadata, HeaderMetadata};
use rp_core::{
...
impl<Block: BlockT> Blockchain<Block> {
	...
	pub fn set_head(&self, hash: Block::Hash) -> rp_blockchain::Result<()> {
		let header = self
			.header(hash)?
			.ok_or_else(|| rp_blockchain::Error::UnknownBlock(format!("{}", hash)))?;

		self.apply_head(&header)
	}

	fn apply_head(&self, header: &<Block as BlockT>::Header) -> rp_blockchain::Result<()> {
		...
		let best_tree_route = {
			...
			if &best_hash == header.parent_hash() {
				...
				let route = rp_blockchain::tree_route(self, best_hash, *header.parent_hash())?;
				Some(route)
			}
		};
	...
	fn append_justification(
		...
		if let Some(stored_justifications) = block_justifications {
			if !stored_justifications.append(justification) {
				return Err(rp_blockchain::Error::BadJustification(
...
impl<Block: BlockT> HeaderBackend<Block> for Blockchain<Block> {
	...
	fn status(&self, hash: Block::Hash) -> rp_blockchain::Result<BlockStatus> {
	...
	fn number(&self, hash: Block::Hash) -> rp_blockchain::Result<Option<NumberFor<Block>>> {
...
impl<Block: BlockT> HeaderMetadata<Block> for Blockchain<Block> {
	...
	fn header_metadata(
		...
		self.header(hash)?
			...
			.ok_or_else(|| {
				rp_blockchain::Error::UnknownBlock(format!("header not found: {}", hash))
			})
	}
...
impl<Block: BlockT> blockchain::Backend<Block> for Blockchain<Block> {
	...
	fn last_finalized(&self) -> rp_blockchain::Result<Block::Hash> {
		Ok(self.storage.read().finalized_hash)
	}

	fn leaves(&self) -> rp_blockchain::Result<Vec<Block::Hash>> {
		Ok(self.storage.read().leaves.hashes())
	}

	fn children(&self, _parent_hash: Block::Hash) -> rp_blockchain::Result<Vec<Block::Hash>> {
		unimplemented!()
	}

	fn indexed_transaction(&self, _hash: Block::Hash) -> rp_blockchain::Result<Option<Vec<u8>>> {
	...
	fn block_indexed_body(
		&self,
		_hash: Block::Hash,
	) -> rp_blockchain::Result<Option<Vec<Vec<u8>>>> {
		unimplemented!("Not supported by the in-mem backend.")
	}
}
...
impl<Block: BlockT> backend::BlockImportOperation<Block> for BlockImportOperation<Block> {
	...
	fn state(&self) -> rp_blockchain::Result<Option<&Self::State>> {
	...
	fn insert_aux<I>(&mut self, ops: I) -> rp_blockchain::Result<()>
	where
	...
	fn mark_head(&mut self, hash: Block::Hash) -> rp_blockchain::Result<()> {
...
impl<Block: BlockT> backend::Backend<Block> for Backend<Block> {
	...
	fn begin_operation(&self) -> rp_blockchain::Result<Self::BlockImportOperation> {
	...
	fn commit_operation(&self, operation: Self::BlockImportOperation) -> rp_blockchain::Result<()> {
	...
	) -> rp_blockchain::Result<Self::State> {
	...
	) -> rp_blockchain::Result<(NumberFor<Block>, HashSet<Block::Hash>)> {
		Ok((Zero::zero(), HashSet::new()))
	}

	fn remove_leaf_block(&self, _hash: Block::Hash) -> rp_blockchain::Result<()> {
		Ok(())
	}
...
pub fn check_genesis_storage(storage: &Storage) -> rp_blockchain::Result<()> {
...
mod tests {
	...
	use rp_blockchain::Backend;
	use rp_runtime::{traits::Header as HeaderT, ConsensusEngineId, Justifications};
	...
	fn store_duplicate_justifications_is_forbidden() {
		...
		assert!(matches!(
			...
			Err(rp_blockchain::Error::BadJustification(_)),
		));
	}
}
