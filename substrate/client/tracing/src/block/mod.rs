...
use rp_api::{Core, ProvideRuntimeApi};
use rp_blockchain::HeaderBackend;
use rp_core::hexdisplay::HexDisplay;
use rp_rpc::tracing::{BlockTrace, Span, TraceBlockResponse};
use rp_runtime::{
	generic::BlockId,
...
pub trait TracingExecuteBlock<Block: BlockT>: Send + Sync {
	...
	fn execute_block(&self, orig_hash: Block::Hash, block: Block) -> rp_blockchain::Result<()>;
}
...
where
	...
	Block: BlockT,
{
	fn execute_block(&self, _: Block::Hash, block: Block) -> rp_blockchain::Result<()> {
		self.client
			.runtime_api()
...
pub enum Error {
	...
	InvalidBlockId(#[from] rp_blockchain::Error),
