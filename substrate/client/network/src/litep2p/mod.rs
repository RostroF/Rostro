...
use rc_network_types::kad::{Key as RecordKey, PeerRecord, Record as P2PRecord};
			...
			tokio::select! {
				...
				event = self.discovery.next() => match event {
					...
					Some(DiscoveryEvent::GetRecordPartialResult { query_id, record }) => {
						...
						let peer_id: rc_network_types::PeerId = record.peer.into();
						let record = PeerRecord {
							record: P2PRecord {
								key: record.record.key.to_vec().into(),
								value: record.record.value,
								publisher: record.record.publisher.map(|peer_id| {
									let peer_id: rc_network_types::PeerId = peer_id.into();
									peer_id.into()
								}),
