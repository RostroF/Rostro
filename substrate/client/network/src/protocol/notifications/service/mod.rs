...
impl NotificationService for NotificationHandle {
	...
	async fn open_substream(&mut self, _peer: rc_network_types::PeerId) -> Result<(), ()> {
		todo!("support for opening substreams not implemented yet");
	}

	/// Instruct `Notifications` to close substream for `peer`.
	async fn close_substream(&mut self, _peer: rc_network_types::PeerId) -> Result<(), ()> {
		todo!("support for closing substreams not implemented yet, call `NetworkService::disconnect_peer()` instead");
	}

	/// Send synchronous `notification` to `peer`.
	fn send_sync_notification(&mut self, peer: &rc_network_types::PeerId, notification: Vec<u8>) {
	...
	fn message_sink(&self, peer: &rc_network_types::PeerId) -> Option<Box<dyn MessageSink>> {
