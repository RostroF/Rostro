...
async fn test_libp2p_litep2p_connectivity() {
	...
	let libp2p_address: rc_network_types::multiaddr::Multiaddr = libp2p_address.clone().into();
...
async fn libp2p_to_litep2p_substream() {
	...
	let address: rc_network_types::multiaddr::Multiaddr = litep2p_address.clone().into();
			...
			event = libp2p.select_next_some() => {
				...
				match event {
					...
					SwarmEvent::Behaviour(NotificationsOut::CustomProtocolClosed { .. }) => {
						...
						let libp2p_peer: rc_network_types::PeerId = libp2p_peer.into();
