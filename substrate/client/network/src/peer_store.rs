...
use rc_network_common::{role::ObservedRole, types::ReputationChange};
use std::{
...
pub trait PeerStoreProvider: Debug + Send + Sync {
	...
	fn outgoing_candidates(
		&self,
		count: usize,
		ignored: HashSet<rc_network_types::PeerId>,
	) -> Vec<rc_network_types::PeerId>;
...
mod tests {
	...
	fn report_banned_peers() {
		let peer_a = rc_network_types::PeerId::random();
		let peer_b = rc_network_types::PeerId::random();
		let peer_c = rc_network_types::PeerId::random();
		...
		handle.report_peer(
			peer_a,
			rc_network_common::types::ReputationChange { value: i32::MIN, reason: "test".into() },
		);
		handle.report_peer(
			peer_b,
			rc_network_common::types::ReputationChange { value: i32::MIN, reason: "test".into() },
