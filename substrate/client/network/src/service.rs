...
//! Main entry point of the rc-network crate.
//!
...
use rc_network_types::kad::{Key as KademliaKey, Record};
	...
	H: ExHashT,
{
	...
	/// Create `PeerStore`.
	fn peer_store(
		bootnodes: Vec<rc_network_types::PeerId>,
	...
	H: ExHashT,
{
	...
	fn external_addresses(&self) -> Vec<rc_network_types::multiaddr::Multiaddr> {
	...
	fn listen_addresses(&self) -> Vec<rc_network_types::multiaddr::Multiaddr> {
	...
	H: ExHashT,
{
	...
	fn find_closest_peers(&self, target: rc_network_types::PeerId) {
		let _ = self
			.to_worker
...
where
	...
	H: ExHashT,
{
	fn set_authorized_peers(&self, peers: HashSet<rc_network_types::PeerId>) {
	...
	fn add_known_address(
		&self,
		peer_id: rc_network_types::PeerId,
		addr: rc_network_types::multiaddr::Multiaddr,
	) {
		let _ = self
			.to_worker
	...
	fn report_peer(&self, peer_id: rc_network_types::PeerId, cost_benefit: ReputationChange) {
	...
	fn disconnect_peer(&self, peer_id: rc_network_types::PeerId, protocol: ProtocolName) {
		let _ = self
			.to_worker
	...
	fn remove_reserved_peer(&self, peer_id: rc_network_types::PeerId) {
	...
	fn set_reserved_peers(
		...
		peers: HashSet<rc_network_types::multiaddr::Multiaddr>,
	) -> Result<(), String> {
	...
	H: ExHashT,
{
	async fn request(
		&self,
		target: rc_network_types::PeerId,
		protocol: ProtocolName,
		request: Vec<u8>,
...
pub(crate) fn ensure_addresses_consistent_with_transport<'a>(
	addresses: impl Iterator<Item = &'a rc_network_types::multiaddr::Multiaddr>,
	transport: &TransportConfig,
) -> Result<(), Error> {
	use rc_network_types::multiaddr::Protocol;
