...
use rc_client_api::{Backend, CallExecutor};
use rc_network::{
...
pub trait TestNetNode: Clone + Future<Output = Result<(), Error>> + Send + 'static {
	...
	fn network(&self) -> Arc<dyn rc_network::service::traits::NetworkService>;
...
pub struct TestNetComponents<TBl: BlockT, TBackend, TExec, TRtApi, TExPool> {
	...
	network: Arc<dyn rc_network::service::traits::NetworkService>,
...
where
	TBl: BlockT,
	TBackend: rc_client_api::Backend<TBl> + Send + Sync + 'static,
	...
	E: ChainSpecExtension + Clone + 'static + Send + Sync,
{
	...
	let source = rc_transaction_pool_api::TransactionSource::External;
