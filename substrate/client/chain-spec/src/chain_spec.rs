	...
	fn assimilate_storage(&self, storage: &mut Storage) -> Result<(), String> {
		match self.genesis.resolve()? {
			...
			Genesis::RuntimeGenesis(RuntimeGenesisInner {
				...
				storage
					.top
					.insert(rp_core::storage::well_known_keys::CODE.to_vec(), code.clone());
			},
			Genesis::RuntimeGenesis(RuntimeGenesisInner {
...
impl From<rp_core::storage::Storage> for RawGenesis {
	fn from(value: rp_core::storage::Storage) -> Self {
		Self {
...
struct RuntimeGenesisInner {
	...
	#[serde(default, with = "rp_core::bytes")]
	code: Vec<u8>,
	...
	fn json_container(&self, raw: bool) -> Result<ChainSpecJsonContainer<E>, String> {
		let raw_genesis = match (raw, self.genesis.resolve()?) {
			(
				...
				storage.top.insert(rp_core::storage::well_known_keys::CODE.to_vec(), code);
...
pub fn update_code_in_json_chain_spec(chain_spec: &mut json::Value, code: &[u8]) -> bool {
	...
	if json_contains_path(&chain_spec, &mut path) {
		...
		#[derive(Serialize)]
		struct Container<'a> {
			#[serde(with = "rp_core::bytes", rename = "0x3a636f6465")]
			code: &'a [u8],
		}
...
pub fn set_code_substitute_in_json_chain_spec(
	...
	let substitutes = json::json!({"codeSubstitutes":{ &block_height.to_string(): rp_core::bytes::to_hex(code, false) }});
