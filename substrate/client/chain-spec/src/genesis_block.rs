...
use codec::Encode;
use rc_client_api::{backend::Backend, BlockImportOperation};
use rc_executor::RuntimeVersionOf;
use rp_core::storage::{well_known_keys, StateVersion, Storage};
use rp_runtime::{
...
pub fn resolve_state_version_from_wasm<E, H>(
	storage: &Storage,
	executor: &E,
) -> rp_blockchain::Result<StateVersion>
where
	E: RuntimeVersionOf,
	H: HashT,
{
	if let Some(wasm) = storage.top.get(well_known_keys::CODE) {
		let mut ext = rp_state_machine::BasicExternalities::new_empty(); // just to read runtime version.

		let code_fetcher = rp_core::traits::WrappedRuntimeCode(wasm.as_slice().into());
		...
		Err(rp_blockchain::Error::VersionInvalid(
...
pub trait BuildGenesisBlock<Block: BlockT> {
	...
	fn build_genesis_block(self) -> rp_blockchain::Result<(Block, Self::BlockImportOperation)>;
}
...
impl<Block: BlockT, B: Backend<Block>, E: RuntimeVersionOf> GenesisBlockBuilder<Block, B, E> {
	...
	pub fn new(
		...
		let genesis_storage =
			build_genesis_storage.build_storage().map_err(rp_blockchain::Error::Storage)?;
