...
pub fn print_sysinfo(sysinfo: &rc_telemetry::SysInfo) {
...
pub fn initialize_hwbench_telemetry(
	telemetry_handle: rc_telemetry::TelemetryHandle,
	hwbench: HwBench,
	...
	async move {
		...
		while connect_stream.next().await.is_some() {
			telemetry_handle.send_telemetry(rc_telemetry::SUBSTRATE_INFO, payload.clone());
		}
	}
}
