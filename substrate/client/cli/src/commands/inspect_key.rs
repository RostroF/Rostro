...
use clap::Parser;
use rp_core::crypto::{ExposeSecret, SecretString, SecretUri, Ss58Codec};
use std::str::FromStr;
...
fn expect_public_from_phrase<Pair: rp_core::Pair>(
	expect_public: &str,
	suri: &str,
...
mod tests {
	...
	fn inspect_with_expected_public_key() {
		...
		let valid_public = rp_core::sr25519::Pair::from_string_with_seed(seed, None)
			.expect("Valid")
			.0
			.public();
		...
		let valid_public_with_password =
			rp_core::sr25519::Pair::from_string_with_seed(&seed_with_password, Some(password))
				.expect("Valid")
				.0
				.public();
		...
		let valid_public_with_password_and_derivation =
			rp_core::sr25519::Pair::from_string_with_seed(
