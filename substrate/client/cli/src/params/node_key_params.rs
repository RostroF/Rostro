		...
		Ok(match self.node_key_type {
			NodeKeyType::Ed25519 => {
				let secret = if let Some(node_key) = self.node_key.as_ref() {
					...
					rc_network::config::Secret::File(key_path)
				};
...
/// Parse a Ed25519 secret key from a hex string into a `rc_network::Secret`.
fn parse_ed25519_secret(hex: &str) -> error::Result<rc_network::config::Ed25519Secret> {
	H256::from_str(hex).map_err(invalid_node_key).and_then(|bytes| {
		ed25519::SecretKey::try_from_bytes(bytes)
			.map(rc_network::config::Secret::Input)
		...
		fn secret_input(net_config_dir: &PathBuf) -> error::Result<()> {
			NodeKeyType::value_variants().iter().try_for_each(|t| {
				...
				params.node_key(net_config_dir, Role::Authority, false).and_then(|c| match c {
					NodeKeyConfig::Ed25519(rc_network::config::Secret::Input(ref ski))
			...
			with_def_params(
				|params| {
					...
					params.node_key(net_config_dir, role, is_dev).and_then(move |c| match c {
						NodeKeyConfig::Ed25519(rc_network::config::Secret::File(ref f))
