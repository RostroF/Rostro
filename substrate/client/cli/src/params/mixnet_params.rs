...
pub struct MixnetParams {
	...
	pub mixnet_session_0_kx_secret: Option<rc_mixnet::KxSecret>,
}

impl MixnetParams {
	/// Returns the mixnet configuration, or `None` if the mixnet is disabled.
	pub fn config(&self, is_authority: bool) -> Option<rc_mixnet::Config> {
		self.mixnet.then(|| {
			let mut config = rc_mixnet::Config {
				core: rc_mixnet::CoreConfig {
