...
use log::info;
use rc_service::{Configuration, Error as ServiceError, TaskManager};
use rc_utils::metrics::{TOKIO_THREADS_ALIVE, TOKIO_THREADS_TOTAL};
...
impl<C: SubstrateCli> Runner<C> {
	...
	/// 2020-06-03 16:14:21 💾 Database: ParityDb at /tmp/c/chains/flamingfir7/paritydb
...
mod tests {
	...
	impl SubstrateCli for Cli {
		...
		fn load_spec(
			&self,
			_: &str,
		) -> std::result::Result<Box<dyn rc_service::ChainSpec>, String> {
			Err("nope".into())
		}
	}
	...
	fn create_runner() -> Runner<Cli> {
		...
		let runner = Runner::new(
			Configuration {
				impl_name: "spec".into(),
				impl_version: "3".into(),
				role: rc_service::Role::Authority,
				tokio_handle: runtime.handle().clone(),
				transaction_pool: Default::default(),
				network: NetworkConfiguration::new_memory(),
				keystore: rc_service::config::KeystoreConfig::InMemory,
				database: rc_client_db::DatabaseSource::ParityDb { path: root.clone() },
				trie_cache_maximum_size: None,
				warm_up_trie_cache: None,
				state_pruning: None,
				blocks_pruning: rc_client_db::BlocksPruning::KeepAll,
				chain_spec: Box::new(
				...
				rpc: RpcConfiguration {
					...
					port: 9944,
					batch_config: rc_service::config::RpcBatchRequestConfig::Unlimited,
					rate_limit: None,
				...
				announce_block: true,
				base_path: rc_service::BasePath::new(root.clone()),
				data_path: root,
			},
			runtime,
			Signals::dummy(),
	...
	fn ensure_run_until_exit_informs_tasks_to_end() {
		...
		runner
			.run_node_until_exit(move |cfg| async move {
				...
				Ok::<_, rc_service::Error>(task_manager)
			})
			.unwrap_err();
