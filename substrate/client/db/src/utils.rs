...
use codec::Decode;
use rc_client_api::blockchain::{BlockGap, BlockGapType};
...
pub fn number_index_key<N: TryInto<u32>>(n: N) -> rp_blockchain::Result<NumberIndexKey> {
	let n = n.try_into().map_err(|_| {
		rp_blockchain::Error::Backend("Block number cannot be converted to u32".into())
	})?;
...
pub fn number_and_hash_to_lookup_key<N, H>(number: N, hash: H) -> rp_blockchain::Result<Vec<u8>>
where
	N: TryInto<u32>,
	H: AsRef<[u8]>,
{
...
pub fn block_id_to_lookup_key<Block>(
	...
	key_lookup_col: u32,
	id: BlockId<Block>,
) -> Result<Option<Vec<u8>>, rp_blockchain::Error>
where
	Block: BlockT,
	::rp_runtime::traits::NumberFor<Block>: UniqueSaturatedFrom<u64> + UniqueSaturatedInto<u64>,
{
	Ok(match id {
...
pub enum OpenDbError {
	...
	DoesNotExist,
	Internal(String),
	DatabaseError(rp_database::error::DatabaseError),
	UnexpectedDbType {
...
impl From<OpenDbError> for rp_blockchain::Error {
	fn from(err: OpenDbError) -> Self {
		rp_blockchain::Error::Backend(err.to_string())
	}
}
...
fn open_kvdb_rocksdb<Block: BlockT>(
	...
	Ok(rp_database::as_rocksdb_database(db))
}
...
pub fn read_db<Block>(
	...
	col: u32,
	id: BlockId<Block>,
) -> rp_blockchain::Result<Option<DBValue>>
where
	Block: BlockT,
{
...
pub fn read_header<Block: BlockT>(
	...
	col: u32,
	id: BlockId<Block>,
) -> rp_blockchain::Result<Option<Block::Header>> {
	match read_db(db, col_index, col, id)? {
		Some(header) => match Block::Header::decode(&mut &header[..]) {
			Ok(header) => Ok(Some(header)),
			Err(_) => Err(rp_blockchain::Error::Backend("Error decoding header".into())),
		},
		None => Ok(None),
	}
}
...
pub fn read_meta<Block>(
	db: &dyn Database<DbHash>,
	col_header: u32,
) -> Result<Meta<<<Block as BlockT>::Header as HeaderT>::Number, Block::Hash>, rp_blockchain::Error>
where
	Block: BlockT,
{
	...
	let load_meta_block = |desc, key| -> Result<_, rp_blockchain::Error> {
...
pub fn read_genesis_hash<Hash: Decode>(
	...
	match db.get(COLUMN_META, meta_keys::GENESIS_HASH) {
		Some(h) => match Decode::decode(&mut &h[..]) {
			Ok(h) => Ok(Some(h)),
			Err(err) => {
				Err(rp_blockchain::Error::Backend(format!("Error decoding genesis hash: {}", err)))
			},
		},
		None => Ok(None),
	}
...
mod tests {
	use super::*;
	use codec::Input;
	use rp_runtime::testing::{Block as RawBlock, MockCallU64, TestXt};
