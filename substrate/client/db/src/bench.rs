...
use rp_runtime::{traits::Hash, StateVersion, Storage};
...
impl<Hasher: Hash> rp_state_machine::Storage<Hasher> for StorageDb<Hasher> {
...
pub struct BenchmarkingState<Hasher: Hash> {
	...
	proof_recorder: Option<rp_trie::recorder::Recorder<Hasher>>,
...
impl<Hasher: Hash> BenchmarkingState<Hasher> {
	...
	pub fn new(
		...
		let state_version = rp_runtime::StateVersion::default();
		let mut root = Default::default();
		let mut mdb = MemoryDB::<Hasher>::default();
		rp_trie::trie_types::TrieDBMutBuilderV1::<Hasher>::new(&mut mdb, &mut root).build();
	...
	pub fn recorder(&self) -> Option<rp_trie::recorder::Recorder<Hasher>> {
...
impl<Hasher: Hash> StateBackend<Hasher> for BenchmarkingState<Hasher> {
	...
	fn register_overlay_stats(&self, stats: &rp_state_machine::StateMachineStats) {
		self.state.borrow().as_ref().map(|s| s.register_overlay_stats(stats));
	}

	fn usage_info(&self) -> rp_state_machine::UsageInfo {
		self.state
			.borrow()
			.as_ref()
			.map_or(rp_state_machine::UsageInfo::empty(), |s| s.usage_info())
	}
...
mod test {
	...
	use rp_state_machine::backend::Backend as _;
	...
	fn iteration_is_also_counted_in_rw_counts() {
		let storage = rp_runtime::Storage {
			top: vec![(
	...
	fn read_to_main_and_child_tries() {
		...
		for _ in 0..2 {
			let child1 = rp_core::storage::ChildInfo::new_default(b"child1");
			let child2 = rp_core::storage::ChildInfo::new_default(b"child2");
