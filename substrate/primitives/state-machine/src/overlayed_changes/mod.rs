...
#[cfg(feature = "std")]
use rp_externalities::{Extension, Extensions, TransactionType};
use rp_trie::{empty_child_trie_root, LayoutV1};
...
impl<H: Hasher> From<rp_core::storage::Storage> for OverlayedChanges<H> {
	fn from(storage: rp_core::storage::Storage) -> Self {
		Self {
...
impl<'a> OverlayedExtensions<'a> {
	...
	pub fn register(
		...
		match self.extensions.entry(type_id) {
			...
			MapEntry::Occupied(_) => Err(rp_externalities::Error::ExtensionAlreadyRegistered),
		}
	}
...
mod tests {
	...
	use rp_core::{traits::Externalities, Blake2Hasher};
