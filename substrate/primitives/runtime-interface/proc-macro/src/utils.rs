...
pub fn generate_runtime_interface_include() -> TokenStream {
	match crate_name("rp-runtime-interface") {
...
/// Generates the access to the `rp-runtime-interface` crate.
pub fn generate_crate_access() -> TokenStream {
	if env::var("CARGO_PKG_NAME").unwrap() == "rp-runtime-interface" {
		quote!(rp_runtime_interface)
	} else {
