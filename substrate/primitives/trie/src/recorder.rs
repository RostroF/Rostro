		...
		match access {
			...
			TrieAccess::Value { hash, value, full_key } => {
				...
				tracing::trace!(
					target: LOG_TARGET,
					hash = ?hash,
					key = ?rp_core::hexdisplay::HexDisplay::from(&full_key),
					"Recording value",
				);
...
mod tests {
	...
	type Recorder = super::Recorder<rp_core::Blake2Hasher>;
	...
	fn recorder_transaction_accessed_keys_works() {
		...
		recorder.start_transaction();
		{
			...
			assert_eq!(
				rp_core::Blake2Hasher::hash(TEST_DATA[0].1),
	...
	fn recorder_ignoring_nodes_works() {
		...
		let mut ignored_nodes = IgnoredNodes::from_storage_proof::<rp_core::Blake2Hasher>(
		...
		let ignored_nodes2 = IgnoredNodes::from_storage_proof::<rp_core::Blake2Hasher>(
			&recorder.drain_storage_proof(),
		);
