...
pub fn new_partial(config: &Configuration) -> Result<Service, ServiceError> {
	...
	// Phase Star B7: type-swapped from `rc_service::new_wasm_executor`. The
	// `config.executor` ExecutorConfiguration fields (heap pages,
	// instantiation strategy, runtime cache size) are wasmtime-specific and
	// don't apply to the PVM path; `RostroCodeExecutor::new()` reads
	// `POLKAVM_*` env vars + workspace defaults instead.
