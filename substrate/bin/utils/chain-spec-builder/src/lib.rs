...
fn process_action<T: Serialize + Clone + Sync + 'static>(
	cmd: &CreateCmd,
	code: &[u8],
	builder: rc_chain_spec::ChainSpecBuilder<T>,
...
fn parse_properties(raw: &String, props: &mut rc_chain_spec::Properties) -> Result<(), String> {
...
pub fn generate_chain_spec_for_runtime(cmd: &CreateCmd) -> Result<String, String> {
	...
	let mut properties = rc_chain_spec::Properties::new();
