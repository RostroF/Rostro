...
use rp_runtime::{DigestItem, Perbill};
use rp_session::MembershipProof;
...
fn report_double_voting_validate_unsigned_prevents_duplicates() {
	...
	ExtBuilder::default().add_authorities(authorities).build_and_execute(|| {
		...
		assert_eq!(
			<Beefy as rp_runtime::traits::ValidateUnsigned>::validate_unsigned(
		...
		assert_ok!(<Beefy as rp_runtime::traits::ValidateUnsigned>::pre_dispatch(&call));
		...
		assert_err!(
			<Beefy as rp_runtime::traits::ValidateUnsigned>::pre_dispatch(&call),
			InvalidTransaction::Stale,
		);
	});
}
