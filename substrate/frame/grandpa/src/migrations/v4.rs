...
pub fn pre_migration<T: crate::Config, N: AsRef<str>>(new: N) {
	...
	let next_key = rp_io::storage::next_key(&twox_128(OLD_PREFIX)).unwrap();
	...
	assert!(
		rp_io::storage::next_key(&twox_128(new.as_bytes())).map_or(
		...
		"unexpected next_key({}) = {:?}",
		new,
		rp_core::hexdisplay::HexDisplay::from(
			&rp_io::storage::next_key(&twox_128(new.as_bytes())).unwrap()
		),
	);
...
pub fn post_migration() {
	...
	assert!(rp_io::storage::next_key(&twox_128(OLD_PREFIX))
