...
//!
//! This pallet contains an offchain worker that turns on-chain statement events into
...
use rp_statement_store::{Proof, Statement};
...
pub mod pallet {
	...
	where
		<Self as frame_system::Config>::AccountId: From<rp_statement_store::AccountId>,
	{
	...
	where
		<T as frame_system::Config>::AccountId: From<rp_statement_store::AccountId>,
	{
	...
	where
		<T as frame_system::Config>::AccountId: From<rp_statement_store::AccountId>,
		rp_statement_store::AccountId: From<<T as frame_system::Config>::AccountId>,
		<T as frame_system::Config>::RuntimeEvent: From<pallet::Event<T>>,
		<T as frame_system::Config>::RuntimeEvent: TryInto<pallet::Event<T>>,
		rp_statement_store::BlockHash: From<<T as frame_system::Config>::Hash>,
	{
	...
	fn collect_statements() {
		...
		for (index, event) in frame_system::Pallet::<T>::read_events_no_consensus().enumerate() {
			if let Ok(Event::<T>::NewStatement { account, mut statement }) = event.event.try_into()
			{
				...
				rp_statement_store::runtime_api::statement_store::submit_statement(statement);
			}
		}
	}
}
