...
fn generate_error_impl(scrate: &TokenStream, enum_ty: OuterEnumType) -> TokenStream {
	...
	quote! {
		impl #enum_name_ident {
			...
			pub fn from_dispatch_error(err: #scrate::rp_runtime::DispatchError) -> Option<Self> {
				let #scrate::rp_runtime::DispatchError::Module(module_error) = err else { return None };
