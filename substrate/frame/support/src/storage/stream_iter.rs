...
/// Implementation of [`codec::Input`] using [`rp_io::storage::read`].
///
...
impl StorageInput {
	...
	fn new(key: Vec<u8>) -> Self {
		...
		let (total_length, exists) =
			if let Some(total_length) = rp_io::storage::read(&key, &mut buffer, 0) {
				(total_length, true)
			} else {
				(0, false)
			};
	...
	fn fill_buffer(&mut self) -> Result<(), codec::Error> {
		...
		if let Some(length_minus_offset) =
			rp_io::storage::read(&self.key, &mut self.buffer[present_bytes..], self.offset)
		{
			let bytes_read =
	...
	fn read_big_item(&mut self, into: &mut [u8]) -> Result<(), codec::Error> {
		...
		if let Some(length_minus_offset) =
			rp_io::storage::read(&self.key, &mut out_remaining, self.offset)
		{
...
mod tests {
	...
	fn reading_invalid_data_from_state() {
		rp_io::TestExternalities::default().execute_with(|| {
			...
			rp_io::storage::set(&TestVecU32::hashed_key(), &data_encoded);
			assert_eq!(
			...
			rp_io::storage::set(&TestVecVecU8::hashed_key(), &data_encoded);
