...
pub fn get<T: Decode + Sized>(key: &[u8]) -> Option<T> {
	rp_io::storage::get(key).and_then(|val| {
...
pub fn put<T: Encode + ?Sized>(key: &[u8], value: &T) {
	value.using_encoded(|slice| rp_io::storage::set(key, slice));
}
...
///
/// Returns [`MultiRemovalResults`](rp_io::MultiRemovalResults) to inform about the result. Once the
...
pub fn clear_prefix(
	...
	// TODO: Once the network has upgraded to include the new host functions, this code can be
	// enabled.
	// rp_io::storage::clear_prefix(prefix, maybe_limit, maybe_cursor)
	use rp_io::{KillStorageResult::*, MultiRemovalResults};
	#[allow(deprecated)]
...
pub fn contains_prefixed_key(prefix: &[u8]) -> bool {
	match rp_io::storage::next_key(prefix) {
...
pub fn get_raw(key: &[u8]) -> Option<Vec<u8>> {
	rp_io::storage::get(key).map(|value| value.to_vec())
}
