...
use rp_runtime::generic::{Digest, DigestItem};
...
pub trait StorageDoubleMap<K1: FullEncode, K2: FullEncode, V: FullCodec> {
	...
	fn remove_prefix<KArg1>(k1: KArg1, limit: Option<u32>) -> rp_io::KillStorageResult
	where
...
pub trait StorageNMap<K: KeyGenerator, V: FullCodec> {
	...
	/// overlay are not taken into account when deleting keys in the backend.
	#[deprecated = "Use `clear_prefix` instead"]
	fn remove_prefix<KP>(partial_key: KP, limit: Option<u32>) -> rp_io::KillStorageResult
	where
		K: HasKeyPrefix<KP>;
...
impl<T, OnRemoval: PrefixIteratorOnRemoval> Iterator for PrefixIterator<T, OnRemoval> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		loop {
			let maybe_next = rp_io::storage::next_key(&self.previous_key)
	...
	fn next(&mut self) -> Option<Self::Item> {
		loop {
			let maybe_next = if self.fetch_previous_key {
				...
				rp_io::default_child_storage::next_key(
...
pub trait StoragePrefixedMap<Value: FullCodec> {
	...
	fn remove_all(limit: Option<u32>) -> rp_io::KillStorageResult {
	...
	fn clear(limit: u32, maybe_cursor: Option<&[u8]>) -> rp_io::MultiRemovalResults {
	...
	fn translate_values<OldValue: Decode, F: FnMut(OldValue) -> Option<Value>>(mut f: F) {
		...
		while let Some(next) =
			rp_io::storage::next_key(&previous_key).filter(|n| n.starts_with(&prefix))
		{
			previous_key = next;
...
pub trait StorageDecodeLength: private::Sealed + codec::DecodeLength {
	...
	fn decode_len(key: &[u8]) -> Option<usize> {
		...
		let mut data = [0u8; 5];
		let len = rp_io::storage::read(key, &mut data, 0)?;
	...
	StorageDoubleMapT: generator::StorageDoubleMap<K1, K2, T>,
{
	...
	>(
		...
		if current < bound {
			...
			rp_io::storage::append(&double_map_key, item.encode());
			Ok(())
		} else {
			Err(())
		}
...
pub fn storage_prefix(pallet_name: &[u8], storage_name: &[u8]) -> [u8; 32] {
	let pallet_hash = rp_io::hashing::twox_128(pallet_name);
	let storage_hash = rp_io::hashing::twox_128(storage_name);
...
mod test {
	...
	use rp_crypto_hashing::twox_128;
