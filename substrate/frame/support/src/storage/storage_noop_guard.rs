...
impl<'a> Default for StorageNoopGuard<'a> {
	fn default() -> Self {
		Self {
			storage_root: rp_io::storage::root(rp_runtime::StateVersion::V1),
...
impl<'a> StorageNoopGuard<'a> {
	...
	pub fn from_error_message(error_message: &'a str) -> Self {
		Self { storage_root: rp_io::storage::root(rp_runtime::StateVersion::V1), error_message }
	}
...
impl<'a> Drop for StorageNoopGuard<'a> {
	fn drop(&mut self) {
		...
		assert_eq!(
			rp_io::storage::root(rp_runtime::StateVersion::V1),
			self.storage_root,
			"{}",
			self.error_message,
		);
