...
pub use rp_core::storage::{ChildInfo, ChildType, StateVersion};
pub use rp_io::{KillStorageResult, MultiRemovalResults};
...
pub fn get<T: Decode + Sized>(child_info: &ChildInfo, key: &[u8]) -> Option<T> {
	match child_info.child_type() {
		ChildType::ParentKeyId => {
			...
			rp_io::default_child_storage::get(storage_key, key).and_then(|v| {
...
pub fn put<T: Encode>(child_info: &ChildInfo, key: &[u8], value: &T) {
	match child_info.child_type() {
		ChildType::ParentKeyId => value.using_encoded(|slice| {
			rp_io::default_child_storage::set(child_info.storage_key(), key, slice)
		}),
	}
}
...
pub fn exists(child_info: &ChildInfo, key: &[u8]) -> bool {
	match child_info.child_type() {
		ChildType::ParentKeyId => {
			rp_io::default_child_storage::exists(child_info.storage_key(), key)
		},
	}
}
...
pub fn kill_storage(child_info: &ChildInfo, limit: Option<u32>) -> KillStorageResult {
	match child_info.child_type() {
		ChildType::ParentKeyId => {
			rp_io::default_child_storage::storage_kill(child_info.storage_key(), limit)
		},
	}
}
...
pub fn clear_storage(
	...
	// enabled.
	// rp_io::default_child_storage::storage_kill(prefix, maybe_limit, maybe_cursor)
	let r = match child_info.child_type() {
		ChildType::ParentKeyId => {
			rp_io::default_child_storage::storage_kill(child_info.storage_key(), maybe_limit)
		},
	};
	use rp_io::KillStorageResult::*;
...
pub fn kill(child_info: &ChildInfo, key: &[u8]) {
	match child_info.child_type() {
		ChildType::ParentKeyId => {
			rp_io::default_child_storage::clear(child_info.storage_key(), key);
		},
	}
}
...
pub fn get_raw(child_info: &ChildInfo, key: &[u8]) -> Option<Vec<u8>> {
	match child_info.child_type() {
		ChildType::ParentKeyId => rp_io::default_child_storage::get(child_info.storage_key(), key),
	}
}
...
pub fn put_raw(child_info: &ChildInfo, key: &[u8], value: &[u8]) {
	match child_info.child_type() {
		ChildType::ParentKeyId => {
			rp_io::default_child_storage::set(child_info.storage_key(), key, value)
		},
	}
}
...
pub fn root(child_info: &ChildInfo, version: StateVersion) -> Vec<u8> {
	match child_info.child_type() {
		ChildType::ParentKeyId => {
			rp_io::default_child_storage::root(child_info.storage_key(), version)
		},
	}
}
...
pub fn len(child_info: &ChildInfo, key: &[u8]) -> Option<u32> {
	match child_info.child_type() {
		ChildType::ParentKeyId => {
			let mut buffer = [0; 0];
			rp_io::default_child_storage::read(child_info.storage_key(), key, &mut buffer, 0)
		},
	}
}
