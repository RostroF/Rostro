...
//!
//! Provides an extension trait for [`rp_core::ecdsa::Public`] to do certain operations.

use rp_core::{crypto::ByteArray, ecdsa::Public};
...
/// `rp_core` due to cyclic dependencies  on `rp_io`.
pub trait ECDSAExt {
...
impl ECDSAExt for Public {
	fn to_eth_address(&self) -> Result<[u8; 20], ()> {
		...
		PublicKey::from_sec1_bytes(self.as_slice()).map_err(drop).and_then(|pub_key| {
			...
			<[u8; 20]>::try_from(
				rp_io::hashing::keccak_256(&uncompressed.as_bytes()[1..])[12..].as_ref(),
			)
			.map_err(drop)
		})
	}
