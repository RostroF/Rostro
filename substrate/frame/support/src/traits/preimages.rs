...
impl<T, H: Hash> Bounded<T, H> {
	...
	/// # Examples
	/// ```
	/// use frame_support::{traits::Bounded, rp_runtime::traits::BlakeTwo256};
	///
...
pub trait QueryPreimage {
	...
	fn len(hash: &<Self::H as rp_core::Hasher>::Out) -> Option<u32>;

	/// Returns the preimage for a given hash. If given, `len` must be the size of the preimage.
	fn fetch(hash: &<Self::H as rp_core::Hasher>::Out, len: Option<u32>) -> FetchResult;

	/// Returns whether a preimage request exists for a given hash.
	fn is_requested(hash: &<Self::H as rp_core::Hasher>::Out) -> bool;

	/// Request that someone report a preimage. Providers use this to optimise the economics for
	/// preimage reporting.
	fn request(hash: &<Self::H as rp_core::Hasher>::Out);

	/// Cancel a previous preimage request.
	fn unrequest(hash: &<Self::H as rp_core::Hasher>::Out);
	...
	fn pick<T>(hash: <Self::H as rp_core::Hasher>::Out, len: u32) -> Bounded<T, Self::H> {
		Self::request(&hash);
...
pub trait StorePreimage: QueryPreimage {
	...
	fn note(bytes: Cow<[u8]>) -> Result<<Self::H as rp_core::Hasher>::Out, DispatchError>;

	/// Attempt to clear a previously noted preimage. Exactly the same as `unrequest` but is
	/// provided for symmetry.
	fn unnote(hash: &<Self::H as rp_core::Hasher>::Out) {
		Self::unrequest(hash)
	}
...
impl QueryPreimage for () {
	type H = rp_runtime::traits::BlakeTwo256;

	fn len(_: &rp_core::H256) -> Option<u32> {
		None
	}
	fn fetch(_: &rp_core::H256, _: Option<u32>) -> FetchResult {
		Err(DispatchError::Unavailable)
	}
	fn is_requested(_: &rp_core::H256) -> bool {
		false
	}
	fn request(_: &rp_core::H256) {}
	fn unrequest(_: &rp_core::H256) {}
}

impl StorePreimage for () {
	const MAX_LENGTH: usize = 0;
	fn note(_: Cow<[u8]>) -> Result<rp_core::H256, DispatchError> {
...
mod tests {
	...
	use rp_runtime::{bounded_vec, traits::BlakeTwo256};
