...
/// * [`Inherents`](rp_inherents) are always executed before any other other signed or unsigned
///   extrinsics.
...
pub trait Hooks<BlockNumber> {
	...
	/// [`rp_io`], namely [`rp_io::offchain`]. This includes exotic operations such as HTTP calls
	...
	/// `rp_io::TestExternalities`. This makes it possible to access the storage.
	fn integrity_test() {}
}
...
pub trait BuildGenesisConfig: rp_runtime::traits::MaybeSerializeDeserialize {
...
pub trait GenesisBuild<T, I = ()>: rp_runtime::traits::MaybeSerializeDeserialize {
	...
	#[cfg(feature = "std")]
	fn build_storage(&self) -> Result<rp_runtime::Storage, String> {
	...
	#[cfg(feature = "std")]
	fn assimilate_storage(&self, storage: &mut rp_runtime::Storage) -> Result<(), String> {
		rp_state_machine::BasicExternalities::execute_with_storage(storage, || {
			self.build();
			Ok(())
		})
	}
