...
use rp_arithmetic::traits::Bounded;
use rp_core::Get;
use rp_io::{hashing::twox_128, storage::clear_prefix, KillStorageResult};
	...
	> crate::traits::OnRuntimeUpgrade for VersionedMigration<FROM, TO, Inner, Pallet, DbWeight>
{
	...
	fn pre_upgrade() -> Result<alloc::vec::Vec<u8>, rp_runtime::TryRuntimeError> {
	...
	T::InCodeStorageVersion: StoreInCodeStorageVersion<T>,
{
	fn migrate(db_weight: &RuntimeDbWeight) -> Weight {
		...
		rp_io::storage::clear(&pallet_version_key(<T as PalletInfoAccess>::name()));
	...
	for RemovePallet<P, DbWeight>
{
	...
	#[cfg(feature = "try-runtime")]
	fn post_upgrade(_state: alloc::vec::Vec<u8>) -> Result<(), rp_runtime::TryRuntimeError> {
		use crate::storage::unhashed::contains_prefixed_key;
...
pub trait SteppedMigration {
	...
	fn transactional_step(
		...
		with_transaction_opaque_err(move || match Self::step(cursor, meter) {
			Ok(new_cursor) => {
				cursor = new_cursor;
				rp_runtime::TransactionOutcome::Commit(Ok(cursor))
			},
			Err(err) => rp_runtime::TransactionOutcome::Rollback(Err(err)),
		})
...
pub trait SteppedMigrations {
	...
	/// Returns `None` if the index is out of bounds.
	#[cfg(feature = "try-runtime")]
	fn nth_pre_upgrade(n: u32) -> Option<Result<Vec<u8>, rp_runtime::TryRuntimeError>>;
	...
	fn nth_post_upgrade(n: u32, _state: Vec<u8>)
		-> Option<Result<(), rp_runtime::TryRuntimeError>>;
...
impl SteppedMigrations for () {
	...
	#[cfg(feature = "try-runtime")]
	fn nth_pre_upgrade(_n: u32) -> Option<Result<Vec<u8>, rp_runtime::TryRuntimeError>> {
		Some(Ok(Vec::new()))
	}
...
impl<T: SteppedMigration> SteppedMigrations for T {
	...
	#[cfg(feature = "try-runtime")]
	fn nth_post_upgrade(n: u32, state: Vec<u8>) -> Option<Result<(), rp_runtime::TryRuntimeError>> {
		if n != 0 {
