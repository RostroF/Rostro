...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn store(origin: OriginFor<T>, remark: Vec<u8>) -> DispatchResultWithPostInfo {
			...
			let content_hash = rp_io::hashing::blake2_256(&remark);
			let extrinsic_index = <frame_system::Pallet<T>>::extrinsic_index()
				.ok_or_else(|| Error::<T>::BadContext)?;
			rp_io::transaction_index::index(extrinsic_index, remark.len() as u32, content_hash);
	...
	pub enum Event<T: Config> {
		...
		Stored { sender: T::AccountId, content_hash: rp_core::H256 },
	}
}
