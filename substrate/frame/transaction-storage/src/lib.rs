...
use rp_runtime::traits::{BlakeTwo256, Dispatchable, Hash, One, Saturating, Zero};
use rp_transaction_storage_proof::{
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn store(origin: OriginFor<T>, data: Vec<u8>) -> DispatchResult {
			...
			let root = rp_io::trie::blake2_256_ordered_root(chunks, rp_runtime::StateVersion::V1);
		...
		pub fn renew(
			...
			rp_io::transaction_index::renew(extrinsic_index, content_hash);
	...
	impl<T: Config> Pallet<T> {
		...
		pub(crate) fn verify_chunk_proof(
			...
			// Verify the tx chunk proof.
				...
				rp_io::trie::blake2_256_verify_proof(
					tx_info.chunk_root,
					&proof.proof,
