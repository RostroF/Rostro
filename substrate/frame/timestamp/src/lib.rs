...
use rp_runtime::traits::{AtLeast32Bit, SaturatedConversion, Scale, Zero};
use rp_timestamp::{InherentError, InherentType, INHERENT_IDENTIFIER};
...
pub mod pallet {
	...
	impl<T: Config> ProvideInherent for Pallet<T> {
		...
		fn check_inherent(
			...
			const MAX_TIMESTAMP_DRIFT_MILLIS: rp_timestamp::Timestamp =
				rp_timestamp::Timestamp::new(30 * 1000);
...
impl<T: Config> UnixTime for Pallet<T> {
	fn now() -> core::time::Duration {
		...
		// `rp_timestamp::InherentDataProvider`.
		let now = Now::<T>::get();
