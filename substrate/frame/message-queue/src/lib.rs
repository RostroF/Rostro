...
//! **Weight Metering**
//!
//! The pallet utilizes the [`rp_weights::WeightMeter`] to manually track its consumption to always
...
use rp_arithmetic::traits::{BaseArithmetic, Unsigned};
use rp_core::{defer, H256};
use rp_runtime::{
	traits::{One, Zero},
...
impl<T: Config> Pallet<T> {
	...
	fn process_message_payload(
		...
		let mut id = rp_io::hashing::blake2_256(message);
...
/// Provides a [`rp_core::Get`] to access the `MEL` of a [`codec::MaxEncodedLen`] type.
...
/// Converts a [`rp_core::Get`] with returns a type that can be cast into an `u32` into a `Get`
