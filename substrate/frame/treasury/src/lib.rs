...
pub mod pallet {
	...
	impl<T: Config<I>, I: 'static> Hooks<SystemBlockNumberFor<T>> for Pallet<T, I> {
		...
		fn try_state(_: SystemBlockNumberFor<T>) -> Result<(), rp_runtime::TryRuntimeError> {
			Self::do_try_state()?;
			Ok(())
		}
	}
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	#[cfg(any(feature = "try-runtime", test))]
	fn try_state_proposals() -> Result<(), rp_runtime::TryRuntimeError> {
	...
	/// > spend.valid_from.
	#[cfg(any(feature = "try-runtime", test))]
	fn try_state_spends() -> Result<(), rp_runtime::TryRuntimeError> {
...
impl<R> rp_runtime::traits::TypedGet for TreasuryAccountId<R>
where
	R: crate::Config,
{
