...
use rp_runtime::traits::BlockNumberProvider;
...
pub mod pallet {
	...
	impl<T: Config<I>, I: 'static> Hooks<SystemBlockNumberFor<T>> for Pallet<T, I> {
		...
		fn try_state(_n: SystemBlockNumberFor<T>) -> Result<(), rp_runtime::TryRuntimeError> {
			Self::do_try_state()?;
			Ok(())
		}
...
impl<T: Config<I>, I: 'static> Polling<T::Tally> for Pallet<T, I> {
	...
	fn create_ongoing(class: Self::Class) -> Result<Self::Index, ()> {
		...
		let dummy_account_id =
			codec::Decode::decode(&mut rp_runtime::traits::TrailingZeroInput::new(&b"dummy"[..]))
		...
		Self::ensure_alarm_at(&mut status, index, rp_runtime::traits::Bounded::max_value());
		ReferendumInfoFor::<T, I>::insert(index, ReferendumInfo::Ongoing(status));
		Ok(index)
	}
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	fn schedule_enactment(
		...
		let ok = T::Scheduler::schedule_named(
			(ASSEMBLY_ID, "enactment", index).using_encoded(rp_io::hashing::blake2_256),
	...
	fn do_try_state() -> Result<(), rp_runtime::TryRuntimeError> {
		ensure!(
	...
	#[cfg(any(feature = "try-runtime", test))]
	fn try_state_referenda_info() -> Result<(), rp_runtime::TryRuntimeError> {
	...
	#[cfg(any(feature = "try-runtime", test))]
	fn try_state_tracks() -> Result<(), rp_runtime::TryRuntimeError> {
		T::Tracks::tracks().try_for_each(|track| {
			TrackQueue::<T, I>::get(track.id).iter().try_for_each(
				|(referendum_index, _)| -> Result<(), rp_runtime::TryRuntimeError> {
					ensure!(
					ReferendumInfoFor::<T, I>::contains_key(referendum_index),
