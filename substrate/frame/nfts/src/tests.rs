...
use rp_core::{bounded::BoundedVec, Pair};
use rp_runtime::{
...
fn validate_signature() {
	new_test_ext().execute_with(|| {
		let user_1_pair = rp_core::sr25519::Pair::from_string("//Alice", None).unwrap();
...
fn pre_signed_attributes_should_work() {
	new_test_ext().execute_with(|| {
		...
		let user_1 = user_1_signer.clone().into_account();
		let user_2 = account(2);
		let user_3_pair = rp_core::sr25519::Pair::from_string("//Bob", None).unwrap();
