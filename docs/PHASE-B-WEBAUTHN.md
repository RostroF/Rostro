...
| B1.7 | Solo-node keyring-path proof | an *enrolled*-passkey WebAuthn extrinsic accepted (keyring path); a passkey-rooted (derived) extrinsic rejected | ✅ PROVEN via **B2a** on a fresh keyring-only node (`:9955`): (1) passkey ENROLLED via `Keyring.enroll_key` with a WebAuthn pop; (2) enrolled passkey signs a remark via the keyring path (accepted); (3) passkey-as-sole-root (derived) REFUSED. Harness = dotwave `labtool webauthn-keyring-probe` (082649b) |
