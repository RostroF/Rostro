...
| dotwave `rust_core` | **[P2 done]** `build_prepared_batch` helper; direct `send_content` → `chat_send_prepared` (cert-auth over batch bytes); `send_content_onion` Deliver drop = `PreparedBatch::encode()`, `chat_send_onion` RPC drops `total_shares`; `fetch_and_decrypt` → `combine_chunks_verified` (dedupe on (msg,index), skip corrupt/incomplete, re-poll). All send flows (chat_send / onion / 2hop / deaddrop / to_pickup) route through the two helpers. Keyless — no session/key state. No FRB regen (no bridged signatures changed). Deps repointed to the chat-chunk worktree (revert to /home/coder/Rostro at merge) |
